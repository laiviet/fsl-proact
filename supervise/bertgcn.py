...
class BertGCNClassifier(nn.Module):
    def __init__(self, vectors, args):
        ...
        self.fc = nn.Sequential(
            nn.Dropout(),
            nn.Linear(4 * 2 * 2 * hidden_size + 768 * self.bert_layer, self.args.n_class)
        )
    ...
    def forward(self, inputs):
        ...
        V = torch.cat([anchor_rep, aspect_lstm, aspect_gcn1, aspect_gcn2, out], dim=1)
        logits = self.fc(V)
        embedding = self.fc2(V)
