...
class GCNClassifier(BaseNet):
    def __init__(self, vectors, args):
        super(GCNClassifier, self).__init__(vectors, args)
        if args.tree == 'full':
            self.first_tree = 'dep'
        ...
        self.lstm = nn.LSTM(self.embedder.hidden_size, self.hidden_size, bidirectional=True, batch_first=True,
                            num_layers=1)
