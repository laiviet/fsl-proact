...
class RAMSDataset(Dataset):
    ...
    def __init__(self,
        ...
        with open('{}.bert.pkl'.format(prefix),'rb') as f:
            self.embedding = pickle.load(f)
        print('Load', len(self.embedding))

        with open('{}.negative.bert.pkl'.format(prefix),'rb') as f:
            self.embedding += pickle.load(f)
        print('Load', len(self.embedding))
    ...
    def preprocess(self, i):
        ...
        if 'emb' in self.features:
            preprocesed['emb'] = self.embedding[i]['emb']
