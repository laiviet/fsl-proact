...
def load_fsl_dl(args):
    feature_set = feature_map[args.encoder]
    TN, N, K, Q = args.train_way, args.way, args.shot, args.query
    train_dl = FSLDataset(TN, K, Q,
...
def main(args):
    ...
    train_dl, dev_dl, test_dl = load_fsl_dl(args)

    bert_model = BertEmbedding()
    params = [x for x in bert_model.parameters() if x.requires_grad == True]
    optimizer = torch.optim.SGD(params, lr=args.lr)
    classifier = meta_opt_net_class[args.model](device=device)
